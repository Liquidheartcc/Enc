...
async def enleech(event, client):
    ...
    if not is_video_file(file.name):
        await event.reply(no_dl_spt_msg)
        await asyncio.sleep(5)
        return
    ...
    async with queue_lock:
        ...
        for item in queue.values():
            if file.name in item:
                await event.reply("**THIS LINK HAS ALREADY BEEN ADDED TO QUEUE**")
                await asyncio.sleep(5)
        ...
        queue.update({(chat_id, (link unavailable)): [file.name, (user_id, None), ("aria2", "None")]})
        await save2db()

        msg = await event.reply(f"**Added to queue ⏰, POS:** `{len(queue)-1}`\n`Please Wait , Encode will start soon`")
        await asyncio.sleep(5)
